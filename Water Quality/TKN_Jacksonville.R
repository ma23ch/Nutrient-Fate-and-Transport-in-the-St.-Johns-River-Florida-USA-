...
fileNamein <- "TKN_Jacksonville_Info.csv"
...
tiff(file.path(outdir, "TKN_Jacksonville_ConcHist.tif"),
...
par(mar = c(4.6, 5.2, 0.8, 0.8), mgp = c(2.6, 0.8, 0), tcl = -0.3,
    ...
    col.lab = "white")  # hide both axis titles while plotting
plotConcHist(eList, printTitle = FALSE)
dev.off()

# ---- Flux histogram (remove y-axis title; keep x-axis title) ----
old_opts <- options(scipen = 999, big.mark = ",")
tiff(file.path(outdir, "TKN_Jacksonville_FluxHist.tif"),
     units = "in", width = 6.5, height = 5.0, res = 600,
     compression = "lzw", pointsize = 14)

op <- par(no.readonly = TRUE)
par(mar = c(4.6, 5.2, 1.0, 0.8), las = 1, tcl = -0.3,
    cex.axis = 1.3, cex.lab = 1.5, family = "Arial",
    col.lab = "white")  # hide both axis titles while plotting

# Draw the plot (y-axis title will be invisible)

# ============================================
# TN ↔ Land-Cover Changes: Full Analysis (1999–2024)
# ============================================
...
tn <- tribble(
  ~Site,         ~TN_conc_change_pct_1999_2024, ~TN_flux_change_pct_1999_2024,
  "Rockledge",                 23,                               0.8,
  "Sanford",                   8.9,                              3.8,
  "Cow Creek",                -26,                              -27,
  "Jacksonville",             -23,                              -25
...
combined   <- landcover %>% left_join(tn, by = "Site")
sites_only <- combined %>% filter(Site != "Full Watershed")

readr::write_csv(combined,   "TN_RawCombinedData_1999_2024.csv")
readr::write_csv(sites_only, "TN_SitesOnly_1999_2024.csv")

# ------------------------------------------------------
# 2) Correlations (Pearson r) WITH p-values (pairwise)
# ------------------------------------------------------
land_cols <- c("Water","Developed","Barren Land","Forest","Agriculture","Wetlands")
targets   <- c("TN_conc_change_pct_1999_2024","TN_flux_change_pct_1999_2024")
...
readr::write_csv(corr_df, "TN_Correlations_with_p_1999_2024.csv")
...
plot_heatmap_singlecol(
  heat_conc,
  "Correlation (r): Land Cover vs TN Concentration % Change (1999–2024)",
  if (SAVE_PNG) "TN_heatmap_r_conc_1999_2024.png" else NULL,
  colors = colorRampPalette(c("navy","white","darkred"))(200)
)
plot_heatmap_singlecol(
  heat_flux,
  "Correlation (r): Land Cover vs TN Flux % Change (1999–2024)",
  if (SAVE_PNG) "TN_heatmap_r_flux_1999_2024.png" else NULL,
...
pca_vars <- c("Water","Developed","Forest","Wetlands",
              "TN_conc_change_pct_1999_2024","TN_flux_change_pct_1999_2024")
...
readr::write_csv(eig_tbl, "TN_PCA_Eigenvalues_1999_2024.csv")
...
readr::write_csv(loadings_export, "TN_PCA_Loadings_PC1_PC3_1999_2024.csv")

scores_tbl <- factoextra::get_pca_ind(pca)$coord %>%
  as.data.frame() %>% rownames_to_column("Site") %>% as_tibble()
readr::write_csv(scores_tbl, "TN_PCA_Scores_1999_2024.csv")
...
p_scree <- fviz_eig(pca, addlabels = TRUE) +
  ...
  ggtitle("Scree Plot (TN PCA, 1999–2024)")
if (SAVE_PNG) {
  ggplot2::ggsave("TN_plot_scree_1999_2024.png", p_scree, width = 7, height = 5, dpi = 300, bg = "white")
} else { print(p_scree) }
...
p_biplot <- fviz_pca_biplot(
  ...
  ggtitle("PCA Biplot: TN & Land-Cover Changes (1999–2024)") +
  xlab("PC1") + ylab("PC2")
if (SAVE_PNG) {
  ggplot2::ggsave("TN_plot_biplot_1999_2024.png", p_biplot, width = 8, height = 6.5, dpi = 300, bg = "white")
} else { print(p_biplot) }
...
# ----------------------------------------
wb <- createWorkbook()
addWorksheet(wb, "TN_RawCombinedData");      writeData(wb, "TN_RawCombinedData", combined)
addWorksheet(wb, "TN_SitesOnly");            writeData(wb, "TN_SitesOnly", sites_only)
addWorksheet(wb, "TN_Correlations");         writeData(wb, "TN_Correlations", corr_df)
addWorksheet(wb, "TN_PCA_Eigenvalues");      writeData(wb, "TN_PCA_Eigenvalues", eig_tbl)
addWorksheet(wb, "TN_PCA_Loadings_PC1_PC3"); writeData(wb, "TN_PCA_Loadings_PC1_PC3", loadings_export)
addWorksheet(wb, "TN_PCA_Scores");           writeData(wb, "TN_PCA_Scores", scores_tbl)
saveWorkbook(wb, "TN_Combined_Analysis_LandCover_1999_2024.xlsx", overwrite = TRUE)

cat("\nOutputs written:\n",
    "- TN_Combined_Analysis_LandCover_1999_2024.xlsx (all tables)\n",
    "- TN_RawCombinedData_1999_2024.csv, TN_SitesOnly_1999_2024.csv\n",
    "- TN_Correlations_with_p_1999_2024.csv\n",
    "- TN_PCA_Eigenvalues_1999_2024.csv, TN_PCA_Loadings_PC1_PC3_1999_2024.csv, TN_PCA_Scores_1999_2024.csv\n",
    "- TN_heatmap_r_conc_1999_2024.png, TN_heatmap_r_flux_1999_2024.png\n",
    "- TN_plot_scree_1999_2024.png, TN_plot_biplot_1999_2024.png\n")
